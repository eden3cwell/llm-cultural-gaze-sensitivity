"""
RQ3 | Output 2: Binned raw mean differences + overall aggregate, both DVs.

Mirrors RQ2 Output 3 with the explicit conditions (gaze_aversion,
direct_gaze_explicit) in place of the implicit labels.

Primary magnitude/direction metric: RAW MEAN DIFFERENCE from baseline per
10pp fixation bin, in DV units (engagement points; delta units). Carries the
sign that the unsigned omnibus eta^2 cannot, and is immune to the
within-bin-variance inflation of binned Cohen's d.

Also retains an OVERALL aggregate per model x condition (mean difference and,
for reference, Cohen's d), reported as a labelled summary only.

Outputs:
    outputs/new_experiment/RQ3/binned_mean_diff.csv
    outputs/new_experiment/RQ3/overall_aggregate.csv
"""
...
from config import (INFERENTIAL_MODELS, EXPLICIT_CONDITIONS,
...
if __name__ == "__main__":
    print("RQ3 | Output 2: Binned mean differences + overall\n" + "=" * 60)
    dfs = load_all()
