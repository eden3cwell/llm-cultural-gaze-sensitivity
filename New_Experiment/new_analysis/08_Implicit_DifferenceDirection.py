"""
08_DifferenceDirection.py


Outputs:
    outputs/RQ2/binned_mean_diff.csv
    outputs/RQ2/overall_aggregate.csv
"""
...
from config import (DATA_DIR, MODEL_FILES, MODEL_LABELS, ALL_MODELS,
                    IMPLICIT_CONDITIONS, BIN_EDGES, BIN_LABELS, out_dir)
...
def cohens_d(a: np.ndarray, b: np.ndarray) -> float:
    ...
    computes this directly, so it stays hand-derived (same situation as the
    Ferguson threshold -- there's no off-the-shelf equivalent)."""
    n_a, n_b = len(a), len(b)
    if n_a < 2 or n_b < 2:
        return np.nan
...
if __name__ == "__main__":
    print("08 | RQ2: Binned + Overall Mean Differences\n" + "=" * 60)
