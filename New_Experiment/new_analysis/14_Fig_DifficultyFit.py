"""
14_Fig_DifficultyFit.py

Baseline difficulty_delta vs. fixation rate.

Output: outputs/Figures/RQ1/fig_difficulty_<model>.png
"""
...
def plot_model(model_key: str) -> None:
    ...
    ax.set_title(f"Fixation vs. Difficulty Delta (Baseline Condition) - {label}",
                 fontsize=11, fontweight="bold")
    ax.set_xlabel("Fixation Rate (%)", fontsize=9)
    ax.set_ylabel("Difficulty Delta", fontsize=9)
    ax.tick_params(labelsize=8)
    ax.set_xlim(0, 100)
    ax.legend(fontsize=7.5, loc="best")

    plt.tight_layout()
    fname = f"fig_difficulty_{label.replace(' ', '_')}.png"
...
def plot_overview(model_keys: list[str], fname: str, suptitle_suffix: str) -> None:
    ...
    for i, model_key in enumerate(ordered):
        ...
        ax.set_ylabel("Difficulty Delta", fontsize=8.5)
        ax.tick_params(labelsize=8)
        ax.set_xlim(0, 100)
        ax.legend(fontsize=8.5, loc="lower right")

    fig.suptitle(f"Fixation vs. Difficulty Delta (Baseline Condition)",
...
if __name__ == "__main__":
    print("14 | Baseline Difficulty Delta Fit (one figure per model)\n" + "=" * 60)
    available = []
    ...
    plot_overview(available, "fig_difficulty_overview_all.png", "All Models")

    core_available = [k for k in available if k in CORE_MODELS]
    plot_overview(core_available, "fig_difficulty_overview_core.png", "Core Models")
