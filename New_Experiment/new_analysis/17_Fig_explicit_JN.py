"""
17_Fig_explicit_JN.py

Polynomial Johnson-Neyman (floodlight) analysis for the explicit framing
conditions (gaze_aversion, direct_gaze_explicit).

Output:
    outputs/Figures/RQ3/fig_jn_<model>.png
    outputs/Figures/RQ3/fig_jn_cross_model_<dv>_all.png
    outputs/Figures/RQ3/fig_jn_cross_model_<dv>_core.png
"""
...
def plot_model(model_key: str) -> None:
    ...
    fig.suptitle(f"Johnson-Neyman: {label} (Explicit Framing)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    fname = f"fig_jn_{label.replace(' ', '_')}.png"
    path = os.path.join(out_dir("Figures", "RQ3"), fname)
...
def plot_cross_model(dv: str, dv_label: str, model_keys: list[str],
    ...
    fig.suptitle(f"JN Cross-Model: Explicit Framing - {dv_label} ({suptitle_suffix})",
...
if __name__ == "__main__":
    print("17 | Johnson-Neyman: Explicit Framing\n" + "=" * 60)
    available = []
    ...
    core_pairs = [("gaze_aversion", "baseline"), ("direct_gaze_explicit", "baseline")]
    ...
    print(f"\nSaved figures to {out_dir('Figures', 'RQ3')}")
